...
class QLearnerTokenBelief:
    """QMIX learner with belief supervision on unseen and reappearing enemy state."""
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            ...
            # Keep the original hidden-enemy belief objective unchanged in spirit.
            if t < batch.max_seq_length - 1:
                belief_mask_t = unseen_mask_t * alive_mask_t
                ...
                belief_mask_t = belief_mask_t * time_agent_mask_t

                belief_loss_sum = belief_loss_sum + (nll_t * belief_mask_t).sum()
                belief_denom = belief_denom + belief_mask_t.sum()
            ...
            if prev_belief_mu is not None:
                reappear_mask_t = (1.0 - prev_enemy_visible) * enemy_visible_t * alive_mask_t
                prev_raw_nll_t, prev_nll_t = self._belief_nll(
                    state_target_t,
                    prev_belief_mu,
                    prev_belief_logvar,
                    prev_belief_u,
                ...
                reappear_loss_sum = reappear_loss_sum + (prev_nll_t * reappear_mask_t).sum()
                reappear_denom = reappear_denom + reappear_mask_t.sum()

            prev_belief_mu = belief_mu_t
            prev_belief_logvar = belief_logvar_t
            prev_belief_u = belief_u_t
