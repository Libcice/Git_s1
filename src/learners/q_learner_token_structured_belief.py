...
class QLearnerTokenStructuredBelief:
    """QMIX learner with structured prior/posterior belief supervision."""
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            ...
            if not self.disable_belief_loss and t < batch.max_seq_length - 1:
                raw_nll_t, nll_t = self._belief_nll(
                    state_target_t,
                    belief_mu_t,
                    belief_logvar_t,
                    belief_u_t,
            ...
            if not self.disable_repr_loss:
                student_slot_q_t = self.mac.get_slot_q_feats()
                ...
                teacher_slot_q_t = self.teacher_repr_encoder(teacher_input_t)

                if t < batch.max_seq_length - 1 and self.belief_repr_coef > 0.0:
                    repr_mask_t = unseen_mask_t * alive_mask_t * time_agent_mask_t
                    repr_error_t = th.nn.functional.smooth_l1_loss(
                        student_slot_q_t,
                        teacher_slot_q_t.detach(),
                        reduction="none",
                    ).mean(dim=-1)
                ...
                teacher_hidden_summary_t = self._masked_mean(
                    teacher_slot_q_t.view(batch.batch_size * self.args.n_agents, self.args.enemy_num, self.hidden_dim),
    ...
    def load_models(self, path):
        ...
        self.teacher_repr_encoder.load_state_dict(
            th.load("{}/teacher_repr.th".format(path), map_location=lambda storage, loc: storage)
        )
