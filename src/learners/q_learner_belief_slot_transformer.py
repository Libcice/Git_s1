...
class QLearnerBeliefSlotTransformer:
    """QMIX learner with explicit belief-slot supervision on unseen enemies."""
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            ...
            unseen_mask_t = (enemy_obs_t.abs().sum(dim=-1) == 0).float()
            alive_mask_t = (state_target_t[..., 0] > 0).float()
            belief_mask_t = unseen_mask_t * alive_mask_t

            diff = state_target_t - belief_mu_t
            d_inv = th.exp(-belief_logvar_t)
            du = d_inv.unsqueeze(-1) * belief_u_t
            a = th.einsum("...dr,...ds->...rs", belief_u_t, du)
            rank = a.shape[-1]
            eye = th.eye(rank, device=a.device, dtype=a.dtype).view(*([1] * (a.dim() - 2)), rank, rank)
            a = a + eye

            base_quad = (diff * d_inv * diff).sum(dim=-1)
            u_t_dinv_x = th.einsum("...dr,...d->...r", belief_u_t, d_inv * diff)
            ...
            logdet = belief_logvar_t.sum(dim=-1) + logdet_a
            raw_nll_t = 0.5 * (quad + logdet)
            nll_t = raw_nll_t / float(self.enemy_state_feat_dim)
            nll_t = nll_t.clamp(max=self.belief_nll_clip)
