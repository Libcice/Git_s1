...
class MAEQLearner:
    ...
    def MT_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None, logger='',write_log=False,lr = 0.0005,batch_size=32,n_repeat=2):
        ...
        action_all_batch = batch["actions"][:,:-1]
        obs_all_batch = batch["obs"][:,:-1]
        state_all_batch = batch["state"][:, :-1]          # (B,T,state_dim)
            ...
            if self.args.masking_type == 'agent' :
                if n_repeat >= 1 : 
                    for agent in range(self.args.n_agents) :
                        ...
                        input_obs, input_action, out_obs, out_action,out_state = self.mac.agent.mae('mix',obs_all_batch[i,except_masking],action_all_batch[i,except_masking]/self.args.n_actions,train=True,agent_num=agent,masking_type=0)

                        loss_1 = loss_fn(input_obs, out_obs)
                        loss_2 = loss_fn(input_action, out_action)
                        loss_3 = loss_fn(state_all_batch[i, except_masking], out_state)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        self.optimiser.step()
        # ===  MT 训练 ===
        if self.args.use_mt_pretrain and t_env < self.args.mt_pretrain_steps:
            mt_loss_val = self.MT_train(batch, t_env ,episode_num)
            self.logger.log_stat("mt_loss", mt_loss_val, t_env)
