...
class QLearnerTokenLatentBelief:
    """QMIX learner for token transformer with prior/posterior latent belief."""
    ...
    def __init__(self, mac, scheme, logger, args):
        ...
        self.belief_loss_coef = getattr(self.args, "belief_loss_coef", 0.002)
        ...
        self.use_repr_alignment = self.belief_repr_coef > 0.0
        self.use_teacher_q = self.belief_teacher_q_coef > 0.0
        self.disable_repr_loss = (not self.use_repr_alignment) and (not self.use_teacher_q)
        ...
        teacher_q_prefix_input_dim = self.ally_state_feat_dim * 2 + self.enemy_state_feat_dim + 2
        self.teacher_q_prefix_encoder = th.nn.Sequential(
            th.nn.Linear(teacher_q_prefix_input_dim, self.hidden_dim * 2),
            th.nn.ReLU(),
            th.nn.Linear(self.hidden_dim * 2, self.hidden_dim * 5),
        )
        self.teacher_q_head = th.nn.Linear(self.hidden_dim * 6, self.args.n_actions)
        self.params += list(self.teacher_repr_encoder.parameters())
        self.params += list(self.teacher_q_prefix_encoder.parameters())
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.disable_belief_loss and self.disable_repr_loss:
            belief_q_alpha = 0.0
        else:
        ...
        for t in range(batch.max_seq_length):
            ...
            if not self.disable_belief_loss and t < batch.max_seq_length - 1:
                raw_nll_t, nll_t = self._belief_nll(state_target_t, belief_mu_t, belief_logvar_t, belief_u_t)
                hidden_mask_t = unseen_mask_t * alive_mask_t * time_agent_mask_t
                hidden_loss_sum = hidden_loss_sum + (nll_t * hidden_mask_t).sum()
                hidden_denom = hidden_denom + hidden_mask_t.sum()
                raw_nll_sum = raw_nll_sum + raw_nll_t.sum()
                raw_nll_count = raw_nll_count + raw_nll_t.new_tensor(float(raw_nll_t.numel()))
                ...
                visible_loss_sum = visible_loss_sum + (nll_t * visible_mask_t).sum()
                visible_denom = visible_denom + visible_mask_t.sum()

            if not self.disable_belief_loss and prev_enemy_visible is not None:
                ...
                _, reappear_nll_t = self._belief_nll(state_target_t, prior_mu_t, prior_logvar_t, prior_u_t)
            ...
            if not self.disable_repr_loss:
                ...
                if t < batch.max_seq_length - 1 and self.use_repr_alignment:
                    repr_mask_t = unseen_mask_t * alive_mask_t * time_agent_mask_t
                    repr_error_t = th.nn.functional.smooth_l1_loss(
                        belief_value_feats_t,
                        teacher_value_feats_t.detach(),
                        reduction="none",
                    ).mean(dim=-1)
                    repr_loss_sum = repr_loss_sum + (repr_error_t * repr_mask_t).sum()
                    repr_denom = repr_denom + repr_mask_t.sum()

                if self.use_teacher_q:
                    ally_mean_agent_t = ally_state_t.mean(dim=1, keepdim=True).expand(-1, self.args.n_agents, -1)
                    enemy_mean_agent_t = enemy_state_t.mean(dim=1, keepdim=True).expand(-1, self.args.n_agents, -1)
                    visible_frac_t = enemy_visible_t.mean(dim=-1, keepdim=True)
                    hidden_frac_t = unseen_mask_t.mean(dim=-1, keepdim=True)
                    teacher_q_prefix_t = self.teacher_q_prefix_encoder(
                        th.cat(
                            [own_state_t, ally_mean_agent_t, enemy_mean_agent_t, visible_frac_t, hidden_frac_t],
                            dim=-1,
                        )
                    )
                    teacher_hidden_mask_t = unseen_mask_t * alive_mask_t
                    teacher_hidden_summary_t = self._masked_mean(
                        ...
                        teacher_hidden_mask_t.view(batch.batch_size * self.args.n_agents, self.args.enemy_num),
                    )
                    teacher_q_input_t = th.cat(
                        [
                            teacher_q_prefix_t.view(batch.batch_size * self.args.n_agents, -1),
                            teacher_hidden_summary_t,
                        ],
                        dim=-1,
                    )
                    teacher_q_t = self.teacher_q_head(teacher_q_input_t).view(batch.batch_size, self.args.n_agents, -1)
        ...
        if self.use_teacher_q:
            ...
            teacher_q_weight = self.belief_teacher_q_coef * min(
                1.0,
                float(t_env) / float(max(1, self.belief_repr_warmup_t)),
            )
        ...
        if self.disable_belief_loss:
            ...
            belief_weight = 0.0
            loss = q_loss + repr_weight * repr_loss + teacher_q_weight * teacher_q_loss
        else:
            ...
            belief_weight = self.belief_loss_coef * min(
                1.0,
                float(t_env) / float(max(1, self.belief_warmup_t)),
            )
    ...
    def cuda(self):
        ...
        self.teacher_q_prefix_encoder.cuda()
    ...
    def save_models(self, path):
        ...
        th.save(self.teacher_q_prefix_encoder.state_dict(), "{}/teacher_q_prefix.th".format(path))
    ...
    def load_models(self, path):
        ...
        teacher_q_prefix_path = "{}/teacher_q_prefix.th".format(path)
        if os.path.exists(teacher_q_prefix_path):
            self.teacher_q_prefix_encoder.load_state_dict(
                th.load(teacher_q_prefix_path, map_location=lambda storage, loc: storage)
            )
        teacher_q_head_path = "{}/teacher_q_head.th".format(path)
        if os.path.exists(teacher_q_head_path):
            self.teacher_q_head.load_state_dict(
                th.load(teacher_q_head_path, map_location=lambda storage, loc: storage)
            )
