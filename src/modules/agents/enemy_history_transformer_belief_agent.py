...
class EnemyHistoryTransformerBeliefAgent(nn.Module):
    ...
    The current-step inputs remain identical to the history-token belief
    agent. Only the historical sequence is reduced to enemy tokens so we
    keep the main belief logic unchanged while shortening the transformer
    sequence.
    """
    ...
    def __init__(self, input_shape, args):
        ...
        self.max_seq_len = 1 + self.current_context_tokens + self.history_steps * self.tokens_per_step
