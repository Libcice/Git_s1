...
class TokenValueCVAEBeliefAgent(nn.Module):
    """Standalone token-history CVAE belief agent with value-aware heads.

    This path intentionally keeps the older token-CVAE architecture fully
    independent from the evolving ``token_cvae_belief`` baseline so that its
    tensor contracts stay stable.
    """

    def __init__(self, input_shape, args):
        super(TokenValueCVAEBeliefAgent, self).__init__()
        self.args = args
