...
class TokenStructuredBeliefAgent(nn.Module):
    """Transformer-memory agent with a structured per-enemy belief decoder.

    The transformer hidden state is the only temporal memory in this path.
    At each step we decode an indexed prior belief for every enemy from the
    previous memory, then apply an observation-conditioned posterior
    correction only where the enemy is visible.
    """

    def __init__(self, input_shape, args):
        super(TokenStructuredBeliefAgent, self).__init__()
        self.args = args
        ...
        self.prior_mlp = nn.Sequential(
            nn.Linear(self.hidden_dim * 3 + self.prior_aux_dim, self.hidden_dim * 4),
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 4, self.hidden_dim),
        )
        self.prior_ln = nn.LayerNorm(self.hidden_dim)

        self.posterior_aux_dim = 2
        self.posterior_delta = nn.Sequential(
            nn.Linear(self.hidden_dim * 3 + self.posterior_aux_dim, self.hidden_dim * 4),
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 4, self.hidden_dim),
        )
        self.posterior_gate = nn.Sequential(
            nn.Linear(self.hidden_dim * 3 + self.posterior_aux_dim, self.hidden_dim),
            nn.ReLU(),
    ...
    def _belief_stats_from_slots(self, slots):
        ...
        belief_u = self.belief_u_head(slots).view(
            -1,
            self.n_enemies,
    ...
    def forward(self, current_step, prev_memory):
        ...
        prior_input = torch.cat(
            [memory_expand, action_expand, self.enemy_index_embed.expand(memory_expand.size(0), -1, -1), prior_aux],
            dim=-1,
        )
        prior_slots = self.prior_ln(self.prior_mlp(prior_input))

        posterior_aux = self._build_q_slot_aux(current_step)
        ...
        corrected_slots = self.posterior_ln(
            prior_slots + self.posterior_gate(posterior_input) * self.posterior_delta(posterior_input)
        )
        ...
        slot_q_feats = F.relu(self.slot_to_q(torch.cat([post_slots, posterior_aux], dim=-1)))
