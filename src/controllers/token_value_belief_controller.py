...
class TokenValueBeliefMAC:
    """MAC for token transformer agents with local per-enemy value belief."""
    ...
    def __init__(self, scheme, groups, args):
        ...
        self.delta_q_per_enemy = None
        self.rel_logits = None
        self.alive_logits = None
        self.hp_preds = None
        self.q_visible = None
        self.delta_q_hidden = None
        self.enemy_weight = None
    ...
    def _write_belief_summary(self, ep_batch, t):
        ...
        belief_summary = th.zeros(bs, self.n_agents, self.args.state_shape, device=device)
        enemy_belief = th.zeros(
            bs,
            self.n_agents,
            self.args.enemy_num,
        ...
        enemy_belief[..., 0] = th.sigmoid(self.alive_logits) * th.relu(self.hp_preds)
        flat_enemy = enemy_belief.reshape(bs, self.n_agents, -1)
        belief_summary[:, :, self.ally_state_dim:self.ally_state_dim + self.enemy_state_dim] = flat_enemy
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        self.delta_q_per_enemy = delta_q_per_enemy.reshape(bs, self.n_agents, self.args.enemy_num, -1)
        self.rel_logits = rel_logits.reshape(bs, self.n_agents, self.args.enemy_num)
        self.alive_logits = alive_logits.reshape(bs, self.n_agents, self.args.enemy_num)
        self.hp_preds = hp_preds.reshape(bs, self.n_agents, self.args.enemy_num)
        self.q_visible = q_visible.reshape(bs, self.n_agents, -1)
        self.delta_q_hidden = delta_q_hidden.reshape(bs, self.n_agents, -1)
        self.enemy_weight = enemy_weight.reshape(bs, self.n_agents, self.args.enemy_num)
    ...
    def get_delta_q_per_enemy(self):
        return self.delta_q_per_enemy

    def get_rel_logits(self):
        return self.rel_logits

    def get_aux_stats(self):
        return self.alive_logits, self.hp_preds

    def get_q_visible(self):
        return self.q_visible

    def get_delta_q_hidden(self):
        return self.delta_q_hidden

    def get_enemy_weight(self):
        return self.enemy_weight
