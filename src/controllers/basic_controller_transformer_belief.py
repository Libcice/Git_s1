...
class BasicMACTransformerBelief:
    """History-token MAC with full-state belief output.

    This keeps the original transformer_belief path separate from the
    default QMIX+RNN implementation.
    """
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        if self.history_tokens is None:
            ...
            self.history_tokens = self.history_tokens[:, :, -getattr(self.args, "history_steps", 4):]

        history = self.history_tokens.reshape(bs * self.n_agents, -1, self.token_dim)
        ...
        self.belief_mu = belief_mu.view(bs, self.n_agents, -1)
        self.belief_logvar = belief_logvar.view(bs, self.n_agents, -1)
        self.belief_u = belief_u.view(bs, self.n_agents, belief_mu.shape[-1], -1)
