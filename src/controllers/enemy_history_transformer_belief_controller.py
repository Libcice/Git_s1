...
class EnemyHistoryTransformerBeliefMAC:
    """MAC for enemy-history transformer belief experiments.

    This path keeps the current-step inputs unchanged, but only stores
    enemy tokens in the short history sequence.
    """
    ...
    def _setup_layout(self, scheme):
        ...
        self.tokens_per_step = self.args.enemy_num
        self.args.history_tokens_per_step = self.tokens_per_step
        self.args.current_context_tokens = 2 + (self.n_agents - 1)
    ...
    def _pack_visible_history(self, history_tokens, history_visible):
        bs, n_agents, steps, n_enemies, token_dim = history_tokens.shape
        flat_tokens = history_tokens.reshape(bs * n_agents, steps * n_enemies, token_dim)
        flat_visible = history_visible.reshape(bs * n_agents, steps * n_enemies) > 0
        lengths = flat_visible.sum(dim=1)
        max_len = int(lengths.max().item()) if lengths.numel() > 0 else 0

        if max_len == 0:
            return flat_tokens[:, :0]

        # Vectorized packing keeps visible-enemy order while avoiding a Python loop per agent.
        packed = flat_tokens.new_zeros(flat_tokens.size(0), max_len, token_dim)
        visible_order = flat_visible.long().cumsum(dim=1) - 1
        row_index = th.arange(flat_tokens.size(0), device=flat_tokens.device).unsqueeze(1).expand_as(flat_visible)
        packed[row_index[flat_visible], visible_order[flat_visible]] = flat_tokens[flat_visible]
        return packed
    ...
    def _build_step_tokens(self, ep_batch, t):
        ...
        enemy_history_tokens = self._pad_tokens(enemy)
        ...
        current_context = th.cat([move_token, self_token_padded, ally_tokens], dim=2)
        ...
        return enemy_history_tokens, enemy_visible, current_context, current
    ...
    def forward(self, ep_batch, t, test_mode=False):
        bs = ep_batch.batch_size
        cache = getattr(ep_batch, "_enemy_history_transformer_cache", None)
        if cache is None:
            cache = {}
            setattr(ep_batch, "_enemy_history_transformer_cache", cache)

        cached_inputs = cache.get(t)
        if cached_inputs is None:
            step_tokens, step_visible, current_context, current = self._build_step_tokens(ep_batch, t)
            step_tokens = step_tokens.unsqueeze(2)
            step_visible = step_visible.unsqueeze(2)
            if self.history_tokens is None:
                self.history_tokens = step_tokens
                self.history_visible = step_visible
            else:
                self.history_tokens = th.cat([self.history_tokens, step_tokens], dim=2)
                self.history_visible = th.cat([self.history_visible, step_visible], dim=2)
                history_keep = getattr(self.args, "history_steps", 4)
                self.history_tokens = self.history_tokens[:, :, -history_keep:]
                self.history_visible = self.history_visible[:, :, -history_keep:]

            history = self._pack_visible_history(self.history_tokens, self.history_visible)
            current_context = current_context.reshape(bs * self.n_agents, -1, self.token_dim)
            history = th.cat([current_context, history], dim=1)
            cached_inputs = (history, current)
            cache[t] = cached_inputs
        else:
            history, current = cached_inputs
