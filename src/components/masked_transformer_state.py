...
class Base_Transformer(nn.Module) :
    def __init__(self, obs_dim, action_dim,state_dim, config,device,positional_type='agent') :
        super().__init__()
        self.batchsize = 6
        self.n_embd       = 64
        self.n_heads      = 4
        ...
        self.decoder_action_embed = nn.Linear(self.n_embd,action_dim).to(self.device)
         ...
         # 新增全局状态分支
        self.decoder_global = nn.Linear(self.n_embd, state_dim)
    ...
    def decode(self, tgt, memory) :
        ...
        # ---------- 新增全局分支 ----------
        T = L // (2 * self.n_agent)                 
        out_4d = out.view(batch_size, T, 2*self.n_agent, D)  
        global_feat = out_4d.mean(dim=2)                     #   (B,T,D)
        global_state_hat = self.decoder_global(global_feat)   
        return output_obs, output_action,global_state_hat
    ...
    def masking(self,masking_ratio,train,ori_data,agent_num,masking_type) :
        ...
        if train :
            if masking_ratio == 'random' :
                ...
                mask = torch.zeros(ori_data.shape[:2], device=ori_data.device, dtype=torch.bool)
                if masking_type != 0 :
        ...
        #     mask = self.execution_masking.repeat(int(batch_size/self.n_agent),1)
            ...
            n_repeats = (batch_size + self.n_agent - 1) // self.n_agent
            mask = self.execution_masking.repeat(n_repeats, 1)[:batch_size]
        if masking_ratio == 'random' :
            mask = mask >= ratio * 100 
        else :
            mask = mask >= 100 
    ...
    def forward(self, ratio,obs, action,train=True,agent_num=1,masking_type=0) :
        ...
        if train is not True :
            ...
            out_obs, out_action, global_state_hat = self.decode(positional_encoding_added_masked_input_2,
                                                                 encoder_return)
            # 推理模式
            return out_obs, out_action, global_state_hat  
        else :
            ...
            out_obs, out_action, global_state_hat = self.decode(positional_encoding_added_masked_input_2,
                                                                 encoder_return)
            # 训练模式多返回一个全局状态
            return input_obs, input_action, out_obs, out_action, global_state_hat
